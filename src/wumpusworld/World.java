...
public class World 
{
    ...
    public  static  final String MARKED = "M";     /*REVISION 1 !!!!*///every coordinate is not MARKED at first
    public static  final String REACH = "R";/*REVISION 5 !!!!*/
    ...
    public World(int size)
    {
        ...
        for (int x = 0; x <= size; x++)
        {
            ...
            {
                w[x][y] = UNKNOWN;//every coordinate is UNKNOWN when the world is initialized
                append(x,y,REACH);/*REVISION 6 !!!!*/
            }
        }
    ...
    private void setVisited(int x, int y)
    {
        ...
        {
            knowns += 1;                                /*REVISION 4!!!!*/
            w[x][y] = w[x][y].replaceAll(UNKNOWN, "");// every time, player visit the coordinate, we remove the UNKNOWN.
        }
    }
    ...
    public boolean hasReach(int x, int y){
        if (!isValidPosition(x,y)) return false;

        if (w[x][y].contains(REACH))
            return true;
        else
            return false;
    }
    public void setUnreachable(int x, int y){
        w[x][y] = w[x][y].replaceAll(REACH, "");
    }
